;;; pseudoscheme-scheme-indent.el --- enhanced scheme-indent mode
...
(require 'pseudoscheme) ; only for its cl-lib loading smartness
(require 'cl-lib)

(defgroup scheme-indent nil
  "Indentation in Lisp."
  :group 'lisp)

(defcustom scheme-indent-maximum-backtracking 6
...
(defcustom scheme-tag-indentation 1
  "Indentation of tags relative to containing list.
This variable is used by the function `scheme-indent-tagbody'."
  :type 'integer
  :group 'scheme-indent)

(defcustom lisp-tag-body-indentation 3
  "Indentation of non-tagged lines relative to containing list.
This variable is used by the function `scheme-indent-tagbody' to indent normal
...

(defvar scheme-indent-defun-method '(4 &lambda &body)
  "Defun-like indentation method.
This applies when the value of the `common-scheme-indent-function' property
is set to `defun'.")
...
OPTIONS are:
 ...
 (:indentation (symbol spec) ...)
  ...
  a normal `common-scheme-indent-function' indentation specification.
...
(define-common-lisp-style "basic-common"
  (:variables
   (scheme-indent-maximum-backtracking 6)
   ...
   (scheme-indent-defun-method (4 &lambda &body))
...
;;;;
;;;; 2. Globally set indentation, from the `common-scheme-indent-function'
...
(defun common-lisp-get-indentation (name &optional full)
  ...
  (let ((method
         (or
          ...
          (get name 'common-scheme-indent-function)
...
;;;###autoload
(defun common-scheme-indent-function (indent-point state)
  "Function to indent the arguments of a Lisp function call.
This is suitable for use as the value of the variable
`scheme-indent-function'.  INDENT-POINT is the point at which the
indentation function is called, and STATE is the
`parse-partial-sexp' state at that position.  Browse the
`scheme-indent' customize group for options affecting the behavior
of this function.

If the indentation point is in a call to a Lisp function, that
function's common-scheme-indent-function property specifies how
this function should indent it.  Possible values for this
property are:

* defun, meaning indent according to `scheme-indent-defun-method';
...
* any other symbol, meaning a function to call.  The function should
  ...
  `common-scheme-indent-function'.  SEXP-COLUMN is the column of
  the open parenthesis of the innermost containing list.
  NORMAL-INDENT is the column the indentation point was
  originally in.  This function should behave like `scheme-indent-259'.
...
\(4 &rest (&whole 2 &rest 1)), meaning:
  ...
  * arguments after the first should be lists, and there may be any number
  ...
  (common-scheme-indent-function-1 indent-point state))
...
(defun common-scheme-indent-function-1 (indent-point state)
  ...
  (let ((normal-indent (current-column)))
    ...
    (let ((depth 0)
          ...
          (containing-form-start (common-scheme-indent-parse-state-start state))
      ...
      (while (and (not calculated)
                  (< depth scheme-indent-maximum-backtracking))
          ...
          (let (tem full function method tentative-defun)
            ...
            (cond ((and (or (eq (char-after (1- containing-sexp)) ?\')
                  ...
                  ((null method)
                   ...
                   (if tentative-defun
                       (setq tentative-calculated
                             (common-scheme-indent-call-method
                              function scheme-indent-defun-method
                  ...
                  ((integerp method)
                   ...
                   ;;  (also compatible with scheme-indent-function)
          ...
          (unless calculated
            (condition-case ()
                (progn (backward-up-list 1)
                       (setq depth (1+ depth)))
              (error
               (setq depth scheme-indent-maximum-backtracking))))))

      (or calculated tentative-calculated
          ;; Fallback.
          ;;
          ;; Instead of punting directly to calculate-scheme-indent we
...
(defun common-scheme-indent-call-method (function method path state indent-point
                                       sexp-column normal-indent)
  (let ((scheme-indent-error-function function))
    (if (symbolp method)
        (funcall method
                 path state indent-point
                 sexp-column normal-indent)
      (scheme-indent-259 method path state indent-point
                       sexp-column normal-indent))))

;; Dynamically bound in common-scheme-indent-call-method.
(defvar scheme-indent-error-function)

(defun scheme-indent-report-bad-format (m)
  (error "%s has a badly-formed %s property: %s"
         ;; Love those free variable references!!
         scheme-indent-error-function 'common-scheme-indent-function m))


;; Lambda-list indentation is now done in SCHEME-INDENT-LAMBDA-LIST.
;; See also `lisp-lambda-list-keyword-alignment',
;; `lisp-lambda-list-keyword-parameter-alignment' and
;; `lisp-lambda-list-keyword-parameter-indentation' -- dvl

(defvar scheme-indent-lambda-list-keywords-regexp
  "&\\(\
...
(defvar scheme-indent-lambda-list-single-arg-keywords-regexp
  "&\\(whole\\|environment\\)\\>"
  "Regular expression matching lambda-list keywords which take a single argument.")

(defun scheme-indent-lambda-list
  ...
  (let (limit)
    (cond
     ((save-excursion
        ...
        (back-to-indentation)
        (setq limit (point))
        (looking-at scheme-indent-lambda-list-keywords-regexp))
          ...
          (save-excursion
            ...
            (let ((key-indent nil)
              ...
              (while (and next (< (point) indent-point))
                (if (looking-at scheme-indent-lambda-list-keywords-regexp)
                   ...
                   (pos
                    (save-excursion
                      ...
                      (if (eolp)
                        ...
                        col))))
              (cond
               ((looking-at scheme-indent-lambda-list-single-arg-keywords-regexp)
...
(defun scheme-indent-259
  ...
  (catch 'exit
    (let* ((p (cdr path))
           ...
           tem tail)
      (if (not (consp method))
          (scheme-indent-report-bad-format method))
      (while n
               ...
               (throw 'exit
                      (cond ((not (common-lisp-looking-back ")"))
                            ...
                            (t
                             ...
                             (list (scheme-indent-lambda-list
                                    indent-point
                                    sexp-column
                                    containing-form-start)
                                   containing-form-start)))))
                          ...
                          (cond (tail
                                ...
                                (t
                                 (funcall tem path state indent-point
                                          sexp-column normal-indent))))))))))))

(defun scheme-indent-tagbody (path state indent-point sexp-column normal-indent)
...
(defun scheme-indent-do (path state indent-point sexp-column normal-indent)
  (if (>= (car path) 3)
      (let ((lisp-tag-body-indentation lisp-body-indent))
        (funcall (function scheme-indent-tagbody)
                 path state indent-point sexp-column normal-indent))
    (funcall (function scheme-indent-259)
             '((&whole nil &rest
...
(defun scheme-indent-defsetf
    (path state indent-point sexp-column normal-indent)
  (list
   (cond
    ;; Inside the lambda-list in a long-form defsetf.
    ((and (eql 2 (car path)) (cdr path))
     (scheme-indent-lambda-list indent-point sexp-column (elt state 1)))
...
;; SCHEME-INDENT-DEFMETHOD now supports the presence of more than one method
;; qualifier and indents the method's lambda list properly. -- dvl
(defun scheme-indent-defmethod
    (path state indent-point sexp-column normal-indent)
  (scheme-indent-259
   (let ((nskip nil))
     (if (save-excursion
...
(defun scheme-indent-function-lambda-hack (path state indent-point
                                         sexp-column normal-indent)
...
(defun scheme-indent-loop (path state indent-point sexp-column normal-indent)
  (if (cdr path)
      normal-indent
    (let* ((loop-start (elt state 1))
      ...
      (cond ((and lisp-loop-indent-subclauses
             ...
             (list (common-scheme-indent-loop-macro-1 state indent-point)
                   (common-scheme-indent-parse-state-start state)))
            (t
...
(defvar common-scheme-indent-clause-joining-loop-macro-keyword
  "\\(#?:\\)?and"
  "Regexp matching 'and', and anything else there ever comes to be like it.")

(defvar common-scheme-indent-indented-loop-macro-keyword
...
(defvar common-scheme-indenting-loop-macro-keyword
  "\\(#?:\\)?\\(when\\|unless\\|if\\)"
...
(defun common-scheme-indent-parse-state-depth (parse-state)
  (car parse-state))

(defun common-scheme-indent-parse-state-start (parse-state)
  (car (cdr parse-state)))

(defun common-scheme-indent-parse-state-prev (parse-state)
...
(defun common-scheme-indent-loop-macro-1 (parse-state indent-point)
  (catch 'return-indentation
    (save-excursion
      ;; Find first clause of loop macro, and use it to establish
      ;; base column for indentation
      (goto-char (common-scheme-indent-parse-state-start parse-state))
      (let ((loop-start-column (current-column)))
        ...
        (let* ((case-fold-search t)
               ...
               (previous-expression-start
                (common-scheme-indent-parse-state-prev parse-state))
          ...
          (if (looking-at common-lisp-body-introducing-loop-macro-keyword)
            ...
            (cond
             ...
             (t
              ...
              (if (or (looking-at common-scheme-indenting-loop-macro-keyword)
                      (looking-at common-lisp-prefix-loop-macro-keyword))
           ...
           ((or (eolp) (looking-at ";"))
            ...
            (if loop-body-p
              ...
              (or (and (looking-at ";") (common-lisp-trailing-comment))
                  default-value)))
           ((looking-at common-scheme-indent-indented-loop-macro-keyword)
            indented-clause-indentation)
           ((looking-at common-scheme-indent-clause-joining-loop-macro-keyword)
...
(defvar common-scheme-indent-if*-keyword
  "threnret\\|elseif\\|then\\|else"
  "Regexp matching if* keywords")

(defun common-scheme-indent-if*
    (path parse-state indent-point sexp-column normal-indent)
  (list (common-scheme-indent-if*-1 parse-state indent-point)
	(common-scheme-indent-parse-state-start parse-state)))

(defun common-scheme-indent-if*-1 (parse-state indent-point)
  (catch 'return-indentation
    (save-excursion
	...
	(common-scheme-indent-if*-advance-past-keyword-on-line)
	(let* ((case-fold-search t)
	  ...
	  (back-to-indentation)
          ...
          (cond
           ((looking-at common-scheme-indent-if*-keyword)
            (setq if*-body-p t)
            ;; Know there's something else on the line (or would
            ;; have been caught above)
            (common-scheme-indent-if*-advance-past-keyword-on-line)
          ...
          (cond
           ((or (eolp) (looking-at ";"))
            ...
            (if if*-body-p
                if*-body-indentation
              default-value))
           ((not (looking-at common-scheme-indent-if*-keyword))
            ;; Clause body ...
            if*-body-indentation)
           (t
            (- (+ 7 if*-start-column)
...
(defun common-scheme-indent-if*-advance-past-keyword-on-line ()
  (forward-word 1)
  (while (and (looking-at "\\s-") (not (eolp)))
    (forward-char 1))
  (if (eolp)
...
(defun common-lisp-init-standard-indentation ()
  (let ((l '((block 1)
             ...
             (defsubst     (as defun))
             (deftype      (as defun))
             (defmethod   scheme-indent-defmethod)
             (defpackage  (4 2))
             ...
             (if          (&rest nil))
             (if*         common-scheme-indent-if*)
             (lambda      (&lambda &rest scheme-indent-function-lambda-hack))
             ...
             (restart-bind (as let))
             (locally 1)
             (loop           scheme-indent-loop)
             (:method        scheme-indent-defmethod) ; in `defgeneric'
             (multiple-value-bind ((&whole 6 &rest 1) 4 &body))
             (multiple-value-call (4 &body))
             (multiple-value-prog1 1)
             (multiple-value-setq (4 2))
             (multiple-value-setf (as multiple-value-setq))
             (named-lambda (4 &lambda &rest scheme-indent-function-lambda-hack))
             (pprint-logical-block (4 2))
             (print-unreadable-object ((&whole 4 1 &rest 1) &body))
             ;; Combines the worst features of BLOCK, LET and TAGBODY
             (prog        (&lambda &rest scheme-indent-tagbody))
             (prog* (as prog))
             (prog1 1)
             (prog2 2)
             (progn 0)
      ...
      (let* ((name (car el))
        ...
        (unless (symbolp name)
          (error "Cannot set Common Lisp indentation of a non-symbol: %s"
                 name))
        (put name 'common-scheme-indent-function indentation)))))
(common-lisp-init-standard-indentation)

(provide 'scheme-indent)
(provide 'pseudoscheme-scheme-indent)

;;; pseudoscheme-scheme-indent.el ends here
